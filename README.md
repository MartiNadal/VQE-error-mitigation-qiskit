...
├── plotting.py           # All publication-quality PDF plots
...
6. **Parity Discard Heatmap** — Measured parity discard fraction vs $(N, h)$ for the `parity` configuration.
7. **Parity Discard Comparison** — Side-by-side heatmaps of parity discard fraction for `parity` vs `ro+parity`, showing how readout correction reduces the effective discard rate.
8. **Parity Discard Delta** — Heatmap of the reduction in discard fraction $(f_\text{parity} - f_\text{ro+parity})$, quantifying where readout correction provides the greatest benefit to parity post-selection.

Two additional plots are generated by `zne_noise_study.py` and saved to `plots/`:

9. **ZNE Lambda Scaling** — $E(\lambda)$ vs noise scale factor $\lambda \in \{1, 3, 5\}$ with Richardson extrapolation fit, for each noise model composition.
10. **ZNE Noise Comparison** — Raw vs ZNE relative error across readout-dominated, balanced, and gate-dominated synthetic noise models; demonstrates that ZNE effectiveness depends on the gate-to-readout error ratio.
...
|---|---|
| Modern laptop (8 cores) | 4–8 hours |
| Desktop (16 cores) | 2–4 hours |
